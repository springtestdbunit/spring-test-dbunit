...
public class DbUnitTestExecutionListener extends AbstractTestExecutionListener {
	...
	private static final String[] COMMON_DATABASE_CONNECTION_BEAN_NAMES = {
			"dbUnitDatabaseConnection", "dataSource" };

	protected static final String CONNECTION_ATTRIBUTE = Conventions
			.getQualifiedAttributeName(DbUnitTestExecutionListener.class,
					"connection");
	...
	public void prepareTestInstance(TestContext testContext) throws Exception {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Preparing test instance "
					+ testContext.getTestClass() + " for DBUnit");
		}

		String databaseConnectionBeanName = null;
		Class<? extends DataSetLoader> dataSetLoaderClass = FlatXmlDataSetLoader.class;

		DbUnitConfiguration configuration = testContext.getTestClass()
	...
	private String getDatabaseConnectionUsingCommonBeanNames(
	...
	private void prepareDatabaseConnection(TestContext testContext,
			String databaseConnectionBeanName) throws Exception {

		Object databaseConnection = testContext.getApplicationContext()
				.getBean(databaseConnectionBeanName);
	...
	private DatabaseDataSourceConnection createDatabaseDataSourceConnection(
	...
	private void prepareDataSetLoader(TestContext testContext,
			Class<? extends DataSetLoader> dataSetLoaderClass) throws Exception {
		try {
	...
	public void beforeTestMethod(TestContext testContext) throws Exception {
		runner.beforeTestMethod(new DbUnitContextAdapter(testContext));
	}

	public void afterTestMethod(TestContext testContext) throws Exception {
		runner.afterTestMethod(new DbUnitContextAdapter(testContext));
	}

	private static class DbUnitContextAdapter implements DbUnitTestContext {

		private TestContext testContext;

		public DbUnitContextAdapter(TestContext testContext) {
			this.testContext = testContext;
		}

		public IDatabaseConnection getConnection() {
			return (IDatabaseConnection) testContext
					.getAttribute(CONNECTION_ATTRIBUTE);
		}

		public DataSetLoader getDataSetLoader() {
			return (DataSetLoader) testContext
					.getAttribute(DATA_SET_LOADER_ATTRIBUTE);
		}
